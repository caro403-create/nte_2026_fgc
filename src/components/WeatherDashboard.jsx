...
import { Cloud, CloudRain, Sun, Wind, Droplets, SunDim, Sunrise, Sunset, Eye, Gauge, Compass, Thermometer } from 'lucide-react';
import HourlyForecastChart from './HourlyForecastChart';
import { translations } from '../utils/translations';

// WMO Weather code mapping
const getWeatherDetails = (code, t) => {
  const codes = {
    0: { text: t.weatherCodes?.[0] || 'Despejado', icon: <Sun className="w-12 h-12 text-yellow-400" /> },
    1: { text: t.weatherCodes?.[1] || 'Mayormente despejado', icon: <SunDim className="w-12 h-12 text-yellow-300" /> },
    2: { text: t.weatherCodes?.[2] || 'Parcialmente nublado', icon: <Cloud className="w-12 h-12 text-gray-300" /> },
    3: { text: t.weatherCodes?.[3] || 'Nublado', icon: <Cloud className="w-12 h-12 text-gray-400" /> },
    45: { text: t.weatherCodes?.[45] || 'Niebla', icon: <Cloud className="w-12 h-12 text-gray-500" /> },
    48: { text: t.weatherCodes?.[48] || 'Niebla escarchada', icon: <Cloud className="w-12 h-12 text-gray-400" /> },
    51: { text: t.weatherCodes?.[51] || 'Llovizna ligera', icon: <CloudRain className="w-12 h-12 text-blue-300" /> },
    53: { text: t.weatherCodes?.[53] || 'Llovizna moderada', icon: <CloudRain className="w-12 h-12 text-blue-400" /> },
    55: { text: t.weatherCodes?.[55] || 'Llovizna densa', icon: <CloudRain className="w-12 h-12 text-blue-500" /> },
    61: { text: t.weatherCodes?.[61] || 'Lluvia leve', icon: <CloudRain className="w-12 h-12 text-blue-400" /> },
    63: { text: t.weatherCodes?.[63] || 'Lluvia moderada', icon: <CloudRain className="w-12 h-12 text-blue-500" /> },
    65: { text: t.weatherCodes?.[65] || 'Lluvia fuerte', icon: <CloudRain className="w-12 h-12 text-blue-600" /> },
    71: { text: t.weatherCodes?.[71] || 'Nieve leve', icon: <Cloud className="w-12 h-12 text-white" /> },
    73: { text: t.weatherCodes?.[73] || 'Nieve moderada', icon: <Cloud className="w-12 h-12 text-white" /> },
    75: { text: t.weatherCodes?.[75] || 'Nieve fuerte', icon: <Cloud className="w-12 h-12 text-white" /> },
    80: { text: t.weatherCodes?.[80] || 'Chubascos leves', icon: <CloudRain className="w-12 h-12 text-blue-400" /> },
    81: { text: t.weatherCodes?.[81] || 'Chubascos', icon: <CloudRain className="w-12 h-12 text-blue-500" /> },
    82: { text: t.weatherCodes?.[82] || 'Chubascos violentos', icon: <CloudRain className="w-12 h-12 text-blue-600" /> },
    95: { text: t.weatherCodes?.[95] || 'Tormenta', icon: <CloudRain className="w-12 h-12 text-purple-400" /> },
    96: { text: t.weatherCodes?.[96] || 'Tormenta con granizo', icon: <CloudRain className="w-12 h-12 text-purple-500" /> },
    99: { text: t.weatherCodes?.[99] || 'Tormenta severa', icon: <CloudRain className="w-12 h-12 text-purple-600" /> },
  };
  return codes[code] || { text: t.weatherCodes?.[0] || 'Desconocido', icon: <Sun className="w-12 h-12 text-gray-400" /> };
};

export default function WeatherDashboard({ point, lang = 'es' }) {
  if (!point || !point.daily) return null;

  const t = translations[lang];
  const currentW = getWeatherDetails(point.weatherCode, t);
  const now = new Date();
    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-6 text-slate-800 pb-10">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 flex flex-col border border-slate-200 shadow-sm">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-slate-500 text-sm">{t.obsLastUpdated}</p>
              </div>
            ...
            <div className="mt-8 flex items-center gap-6">
              ...
              <div className="ml-4 flex flex-col justify-center">
                ...
                <span className="text-slate-500 font-medium mt-1">
                  {t.obsMax} {Math.round(point.daily.temperature_2m_max[0])}° {t.obsMin} {Math.round(point.daily.temperature_2m_min[0])}°
                </span>
              </div>
            </div>
          </div>
          ...
          <HourlyForecastChart baseTemp={point.temp} lang={lang} />
        ...
        <div className="w-full lg:w-[45%] grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
              <h4 className="text-slate-500 text-sm font-semibold flex items-center gap-2"><Eye className="w-4 h-4" /> {t.obsVisibility}</h4>
              ...
              <p className="text-emerald-500 text-sm font-bold mt-2">{t.obsVisibilityGood}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
              <h4 className="text-slate-500 text-sm font-semibold flex items-center gap-2"><Gauge className="w-4 h-4" /> {t.obsPressure}</h4>
              <div className="mt-2 text-slate-800">
              ...
              <p className="text-blue-500 text-sm font-bold mt-2">{t.obsPressureStable}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
              <h4 className="text-slate-500 text-sm font-semibold flex items-center gap-2"><Sun className="w-4 h-4" /> {t.obsUVIndex}</h4>
              <div className="mt-2 text-slate-800">
                <span className="text-3xl font-extrabold">{point.daily?.uv_index_max?.[0] !== undefined ? Math.round(point.daily.uv_index_max[0]) : '--'}</span>
              </div>
              <p className="text-amber-500 text-sm font-bold mt-2">{t.obsUVModerate}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
              <h4 className="text-slate-500 text-sm font-semibold flex items-center gap-2"><Wind className="w-4 h-4" /> {t.obsAQI}</h4>
              <div className="mt-2 text-slate-800">
                <span className="text-3xl font-extrabold">{point.pm2_5 !== undefined ? point.pm2_5 : '--'}</span>
              </div>
              <p className="text-emerald-500 text-sm font-bold mt-2">{t.obsAQIGood}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm col-span-1 md:col-span-1 relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <h4 className="text-slate-500 text-sm font-semibold mb-4 flex items-center gap-2"><Compass className="w-4 h-4" /> {t.obsTabWind}</h4>
            <div className="flex gap-4 items-center h-full">
              ...
              <div className="flex flex-col gap-3">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 uppercase font-bold">{t.obsWindSpeed}</p>
                </div>
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 uppercase font-bold">{t.obsWindGusts}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <h4 className="text-slate-500 text-sm font-semibold mb-4 flex items-center gap-2"><Droplets className="w-4 h-4" /> {t.obsHumedad}</h4>
            <div className="flex items-end gap-6 h-full pb-4">
              ...
              <div className="flex flex-col items-end">
                ...
                <span className="text-blue-500 font-bold text-sm">{t.obsHumid}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
          <h4 className="text-slate-500 text-sm font-semibold mb-4 flex items-center gap-2"><Sunrise className="w-4 h-4" /> {t.obsSunMoon}</h4>
          ...
          <div className="flex justify-between mt-3 text-slate-800 font-bold text-lg">
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-normal">{t.obsSunrise}</p>
            </div>
            <div className="text-right">
              {formatTime(point.daily?.sunset?.[0])}
              <p className="text-[10px] text-slate-400 font-normal">{t.obsSunset}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <h4 className="text-slate-500 text-sm font-semibold mb-4 flex items-center gap-2"><CloudRain className="w-4 h-4" /> {t.obsTabPrecip}</h4>
            <div className="flex flex-col items-center justify-center h-full pb-4 text-slate-800">
               ...
               <p className="text-blue-500 font-bold text-sm mt-2">{point.rain > 0 ? t.obsRainy : t.obsDry}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <h4 className="text-slate-500 text-sm font-semibold mb-4 flex items-center gap-2"><Thermometer className="w-4 h-4" /> {t.obsSensacion}</h4>
            <div className="flex flex-col items-center justify-center h-full pb-4 text-slate-800">
               <span className="text-4xl font-extrabold">{point.apparent_temp !== undefined ? Math.round(point.apparent_temp) : '--'}°</span>
               <p className="text-amber-500 font-bold text-sm mt-2">{t.obsCurrentFeel}</p>
            </div>
          </div>
        </div>
